...
function headline($title, $mainframe = 1)
{
    ...
    if ((isset($config['interface_server_caption'])) && (1 == $config['interface_server_caption'])) {
        if ($config['interface_server_caption_position'] == $mainframe) {
            $s .= '<div id="server'.$mainframe.'">'.$lang['L_SERVER'].': <a class="server" href="'.getServerProtocol().$_SERVER['SERVER_NAME'].'" target="_blank" rel="noopener" title="'.$_SERVER['SERVER_NAME'].'">'.$_SERVER['SERVER_NAME'].'</a></div>';
        }
    }
    if (1 == $mainframe) {
